...
request. Debug cleartext is enabled solely for this loopback instrumentation path; release manifests are unchanged.
...
```text
KMP06_COIL_CACHE coldMs=616 warmMs=12 coldSource=NETWORK warmSource=DISK requests=1
```
...
| `verifyKmpAndroidCompilerFlags` | Pass; 20 Compose-KMP Android compile tasks, Android-only `-Xlambdas=class` |
| `verifyDesignTokensLogFiles` | Pass; 303 production Kotlin files checked, including `commonMain` and `androidMain` |
| `check -PverifyDesignTokensLogFiles=true --continue --no-parallel ...` | Pass in 49s; 1,651 actionable tasks (106 executed, 1,545 up-to-date) |
| `:app:assembleTmdbDebug :app:assembleClientBDebug --no-parallel ...` | Pass in 1m; 816 actionable tasks |
...
The connected Coil cache suite is one additional device test outside the 265 host-test inventory. No expected host-test task executed zero tests.
`:core:ui` is intentionally test-source-free; the Android cache behavior is exercised by the executable app instrumentation test.

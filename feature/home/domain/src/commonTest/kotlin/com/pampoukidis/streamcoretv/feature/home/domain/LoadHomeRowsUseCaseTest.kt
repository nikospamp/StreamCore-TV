...
class LoadHomeRowsUseCaseTest {
    ...
    @Test
    fun `module resolves home use case with a repository contract`() {
        val repository = FakeHomeRepository(AppResult.Success(emptyList()))
        val application = koinApplication {
            modules(
                module {
                    single<HomeRepository> { repository }
                },
                homeDomainModule,
            )
        }

        kotlin.test.assertIs<LoadHomeRowsUseCase>(application.koin.get<LoadHomeRowsUseCase>())
    ...
    @Test
    fun `propagates repository failure without mapping`() {
        runTest {
            val failure = AppResult.Failure(AppError.Network())
            val repository = FakeHomeRepository(failure)
            val subject = LoadHomeRowsUseCase(repository)

            val result = subject("profile-2")

            assertEquals("profile-2", repository.requestedProfileId)
